...
结论:  设置样式 ,将 样式相同的标签(元素)分为一组, 然后分别设置 css-----(使用样式): style
...
![浮动](C:\Users\Lenovo\Desktop\编程笔记\CSS\笔记\media\C%5CUsers%5CLenovo%5CDesktop%5C%E7%BC%96%E7%A8%8B%E7%AC%94%E8%AE%B0%5CCSS%5Cmedia%5C%E6%B5%AE%E5%8A%A8.png)
...
>
> 若没有已经定位的祖先元素, 则以body (即屏幕左上角)为基准点对齐
>
> /*不占位置, 和浮动一样*/
       ...
       <style>
           * {
           ...
           .tb ul {
               ...
               /*子绝父相, 距离左面 50% , margin-left = - 盒子宽度的一半*/
               position: absolute; /*加定位*/
               left: 50%; /*水平走父容器的一半*/
               bottom: 18px;
               margin-left: -35px; /*左走自己 宽度的一半*/
    ...
    <style>
        * {
        ...
        div {
            ...
            background-color: red;
            position: absolute;
            /*z-index: 0; 只有定位的盒子才有这个属性 , 没有定位的盒子不能用*/
        }
...
```
加了 绝对定位的盒子自动转换为行内块    行内块 的大小根据内容决定    所以 加了定位要有宽高注意子绝父相
```
...
<head>
    ...
    <style>
        div {
            ...
            /*position: absolute; 模式转换 inline-block*/
            position: fixed;
            top: 0;
            left: 0;
            /*模式转换 inline-block*/
...
``` html
  <style>
        ...
        span{
            /*  文字和图片使用的时候 ， 图片： inline-block   vertical: middle */
            display: inline-block;
    ...
    <title>Title</title>
       <style>
           @font-face {
               ...
               src: url('fonts/icomoon.eot?2w9o4q');/*fonts 文件夹的路径*/
               src: url('fonts/icomoon.eot?2w9o4q#iefix') format('embedded-opentype'),
               url('fonts/icomoon.ttf?2w9o4q') format('truetype'),
               url('fonts/icomoon.woff?2w9o4q') format('woff'),
               url('fonts/icomoon.svg?2w9o4q#icomoon') format('svg');
               font-weight: normal;
...
``` html
 <style>
       ...
       /*    css3 的盒子模型 内减模式  , 盒子宽度高度一给定,  指定 padding 盒子的宽度不受变化 , 始终是 300px 300px*/
       }
    </style>
...
<head>
    ...
    <style>
        ...
        div:hover::after{
            /*鼠标经过div 的时候 在里面插入一个 盒子*/
            content: '';

            /*边框压到盒子上只能定位*/
            position: absolute;
            /*background-color: purple;*/
            /*定位转换为行内块  , 没有内容  所以要给宽高 100%*/
            width: 100%;
            height: 100%;
            top: 0;
            left: 0;
            border: 20px solid rgba(255,255,255,.5);
            /*给 border 会撑开盒子 ,不想让边框撑开盒子 内减模式 */
            box-sizing: border-box;
        }
    </style>
</head>
